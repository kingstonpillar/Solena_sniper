...
async function getTokenAccountsByMint(mint) {
  try {
    ...
    for (const a of rawAccounts) {
      try {
        // a.address may be string or pubkey-like; always pass to PublicKey when needed
        const acctInfo = await conn.getParsedAccountInfo(new PublicKey(a.address)).catch(() => null);
        const uiAmount = safeNumber(acctInfo?.value?.data?.parsed?.info?.tokenAmount?.uiAmount, 0);
        const owner = acctInfo?.value?.data?.parsed?.info?.owner || null;
        result.push({
          owner,
          amount: Number(uiAmount),
          rawAmount: a.amount ?? "0",
          address: (a.address?.toString?.() ?? String(a.address))
        });
      } catch {
...
async function getOnchainBuyVolume(mint, limit = 2000) {
  try {
    ...
    const sigInfos = await conn.getSignaturesForAddress(mintPub, { limit: Math.min(limit, 1000) }).catch(() => []);
...
async function analyzePoolActivity(mint, mintCreatorAddress) {
  try {
    ...
    if (!mintPub) return { safe: true, reason: null, details: { scanned: 0, note: "invalid mint" } };

    // scan signatures for the mint
    const sigInfos = await conn.getSignaturesForAddress(mintPub, { limit: Math.min(POOL_SIG_SCAN_LIMIT, 500) }).catch(() => []);
    ...
    const keywordsLPAdd = ["add_liquidity", "mintto", "mint_to", "sync", "deposit"];
    ...
    const poolAgeSecs = (typeof firstPoolEventAt === "number") ? (now - firstPoolEventAt) : null;

    // Suspicious: pool very new
    ...
    // Suspicious: LP added by mint creator quickly
    ...
    return { safe: true, reason: null, details: { poolAgeSecs, poolCreationEvents, lpAddEvents, firstPoolEventBy, scanned } };
  } catch (err) {
...
async function analyzeDeveloperWallet(deployerAddress) {
  try {
    ...
    const deployerPub = (() => { try { return new PublicKey(deployerAddress); } catch { return null; } })();
    if (!deployerPub) return { safe: false, score: 0, reason: "Invalid deployer address", details: {} };

    const sigInfos = await conn.getSignaturesForAddress(deployerPub, { limit: Math.min(DEV_SIG_SCAN_LIMIT, 1000) }).catch(() => []);
    if (!Array.isArray(sigInfos) || sigInfos.length === 0) {
      return { safe: false, score: 0, reason: "Deployer has no on-chain history", details: { sigs: 0 } };
    }

    // signatures are returned newest-first; earliest is last element
    ...
    try {
      const bal = await conn.getBalance(deployerPub).catch(() => null);
...
export async function verifyTokenSecurity(mint) {
  ...
  try {
    ...
    const devAnalysis = await analyzeDeveloperWallet(deployerAddress).catch(() => ({ safe: false, score: 0, reason: "dev analysis failed", details: {} }));
