...
const localRPCLimiter = new PQueue({
  intervalCap: Number(process.env.TOKENSEC_RPC_INTERVAL_CAP || 3), // default 3 RPC calls per second
  ...
  try {
    ...
    for (const a of rawAccounts) {
      try {
        ...
        const acctInfo = await rpcLimited(() => conn.getParsedAccountInfo(new PublicKey(a.address))).catch(() => null);
...
async function analyzePoolActivity(mint, mintCreatorAddress) {
  try {
    ...
    const sigInfos = await rpcLimited(() => conn.getSignaturesForAddress(mintPub, { limit: Math.min(POOL_SIG_SCAN_LIMIT, 500) })).catch(() => []);
...
async function analyzeDeveloperWallet(deployerAddress) {
  try {
    ...
    const sigInfos = await rpcLimited(() => conn.getSignaturesForAddress(deployerPub, { limit: Math.min(DEV_SIG_SCAN_LIMIT, 1000) })).catch(() => []);
    if (!Array.isArray(sigInfos) || sigInfos.length === 0) {
