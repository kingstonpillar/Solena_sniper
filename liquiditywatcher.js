...
const DEX_PROGRAMS = [RAYDIUM, ORCA, METEORA];
...
async function handleAutoBuy(mintAddress) {
  ...
  try{
    await StartWatcher();
    await executeSwap("SOL", mintAddress);
    await StopWatcher();
...
// -----------------------------------------------
// WebSocket pause/resume per DEX program
function createWS(url, dexId) {
  ...
  ws.on("open", () => {
    ws.send(JSON.stringify({
      ...
      params:[{ mentions:[dexId] }, { commitment:"confirmed" }]
    }));
  });
  ...
  ws.on("close", () => { setTimeout(() => createWS(url, dexId), 1000); });
...
function pauseWS() {
  ...
  wsInstances = [];
  console.log("WebSockets paused due to max active positions.");
}

function resumeWS() {
  console.log("WebSockets resuming after all sells complete.");
  DEX_PROGRAMS.forEach(dexId => {
    CHAINSTACK_WSS.forEach(url => createWS(url, dexId));
  });
}

// Listen to swapexecutor signals
StopWatcher.on("maxActiveReached", pauseWS);
StartWatcher.on("allSellComplete", resumeWS);
...
console.log("Liquidity watcher started: one WS per DEX, pause/resume via StartWatcher/StopWatcher.");