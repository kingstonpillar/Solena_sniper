...
if (!liquidityData) {
await sendTelegram("❌ Transaction ${signature} skipped — no valid liquidity detected.");
return;
}
...
if (!mint) {
await sendTelegram("❌ Transaction ${signature} skipped — could not identify mint.");
return;
}

if (migrators.has(mint)) {
await sendTelegram("❌ Token ${mint} ignored — already in migrators list.");
return;
}

const cooldown = getDexCooldown(dex);
if (recentlyTriggered.has(mint) && Date.now() - recentlyTriggered.get(mint) < cooldown) {
await sendTelegram("❌ Token ${mint} skipped — cooldown active for DEX ${dex}.");
return;
}
...
await sendTelegram("❌ Token ${mint} rejected — pool USD $${poolValueUSD?.toFixed(2) || 0} < dynamic min $${dynamicMinUSD}.");
return;
}

// Passed all checks — save and trigger buy
console.log("📊 Liquidity detected — Mint: ${mint}, DEX: ${dex}, Pool USD: $${poolValueUSD.toFixed(2)}");
saveMigrator(mint);
...
// -----------------------------------------------
// -------------------------------
// Multi-WS setup (with connection log, no pause/resume)
const wsInstances = [];

function createWS(url, programId) {
...
ws.on("open", () => {
  reconnectDelay = 1000;
  console.log(`✅ Connected to WS: ${url} (Program: ${programId})`);
...
ws.on("close", () => {
  console.log(`🔴 WS disconnected — retrying in ${reconnectDelay / 1000}s (Program: ${programId})`);
  setTimeout(connect, reconnectDelay);
  reconnectDelay = Math.min(reconnectDelay * 2, MAX_RECONNECT);
});

ws.on("error", err => {
  console.log(`❌ WS Error (Program: ${programId}):`, err.message);
  try { ws.close(); } catch {}
});
...
// Startup: one WS per program ID per endpoint
CHAINSTACK_WSS.forEach(url => {
DEX_PROGRAMS.forEach(programId => {
createWS(url, programId);
});
});

console.log("Liquidity watcher started — WS connections initializing per program ID...");