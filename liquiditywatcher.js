...
const WSOL = "So11111111111111111111111111111111111111112"; // liquidity detection
const SOL = "So11111111111111111111111111111111111111112";  // trading placeholder
...
async function fetchSolPriceUSD() {
  const now = Date.now();
  if (cachedSolPrice && (now - lastSolPriceFetch) < SOL_PRICE_CACHE_MS) return cachedSolPrice;
  try {
...
async function fetchTokenPriceUSD(mint) {
  ...
  try {
    const r = await fetch(`https://public-api.birdeye.so/public/price?address=${mint}`, { headers: { "x-chain":"solana" }});
...
// SOL liquidity detector using WSOL
function detectSolLiquidityFromTx(tx, solPriceUSD) {
  ...
  if (!mints.includes(WSOL) || mints.length!==2) return false;
...
async function handleAutoBuy(mintAddress) {
  ...
  let safeToken=false, safeCreator=false;
  for(let i=0;i<3;i++){ try{ const ts=await verifyTokenSecurity(mintAddress); if(ts?.safe){ safeToken=true; break; } } catch{} await delay(4000);}
  for(let i=0;i<3;i++){ try{ const cs=await verifyCreatorSafety(mintAddress); if(cs?.safe){ safeCreator=true; break; } } catch{} await delay(4000);}
  ...
  while(((priceNow-price0)/price0*100)<MOMENTUM_MIN_PCT && Date.now()-start<MOMENTUM_MAX_WAIT_MS){ await delay(10000); const tmp = await fetchTokenPriceUSD(mintAddress); if(tmp&&tmp>0) priceNow=tmp; }
  if(((priceNow-price0)/price0*100)<MOMENTUM_MIN_PCT){ isBuying=false; return; }

  try{
    await StartWatcher();                // resume WS if paused
    await executeSwap(SOL, mintAddress); // trade SOL
    await StopWatcher();                 // emit maxActiveReached if needed
    removeMigrator(mintAddress);
    await sendTelegram(`✅ BOUGHT\n<code>${mintAddress}</code>\nPrice move: ${((priceNow-price0)/price0*100).toFixed(2)}%`);
  } catch(err){ await sendTelegram(`❌ Buy failed for ${mintAddress}: ${err.message}`); }
...
function createWS(url) {
  ...
  ws.on("open", () => {
    ws.send(JSON.stringify({ jsonrpc:"2.0",id:1,method:"logsSubscribe", params:[{ mentions:Array.from(DEX_PROGRAMS) }, { commitment:"confirmed" }] }));
  });
  ...
  ws.on("close", () => { setTimeout(()=>createWS(url), 1000); });
...
function pauseWS() {
  ...
  wsInstances = [];
  console.log("WebSocket paused due to max active positions.");
}

function resumeWS() {
  console.log("WebSocket resuming after all sells complete.");
  CHAINSTACK_WSS.forEach(url => createWS(url));
}

// -----------------------------------------------
// Listen to swapexecutor signals
StopWatcher.on("maxActiveReached", pauseWS);   // pause when max active positions reached
StartWatcher.on("allSellComplete", resumeWS); // resume when all sells complete
...
CHAINSTACK_WSS.forEach(url => createWS(url));
console.log("Liquidity watcher started with WS pause/resume via StartWatcher/StopWatcher.");