...
async function detectSolLiquidityFromTx(tx, solPrice) {
  ...
  // Determine representative mint: pick the token with highest USD value
  let repMint = null;
  let maxValue = -1;
  for (const mint of mints) {
    ...
    if (mint === WSOL) {
      ...
      if (priceSOL != null) priceUSD = priceSOL * solPrice;
    }
  ...
  for (const mint of mints) {
    ...
    if (priceUSD != null) poolValueUSD += amount * priceUSD;
  }

  if (!poolValueUSD || poolValueUSD <= 0) return null;
...
if (process.argv.includes("--mock-autobuy")) {
  ...
  global.StartWatcher = async () =>
    console.log("[MOCK] StartWatcher called");

  global.StopWatcher = async () =>
    console.log("[MOCK] StopWatcher called");
  ...
  writeMigratorsFile([
    {
      mintAddress: "MockMint111111111111111111111111111111111",
      detectedAt: Date.now()
    }
  ]);

  migrators.add("MockMint111111111111111111111111111111111");
