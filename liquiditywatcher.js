...
import { getOnchainPrice } from "./jupiterOnchain.js";
...
const VOL_SAMPLE_SHORT_MS = Number(process.env.VOL_SAMPLE_SHORT_MS || 60_000); // default 60s
...
async function fetchSolPriceUSD() {
  ...
  try {
    // quote 1 SOL in lamports
    const oneSolLamports = 1_000_000_000;
    const quote = await getOnchainPrice(SOL_MINT, USDC_MINT, oneSolLamports);
...
// ----------------------------- TOKEN PRICE (ON-CHAIN via Jupiter) -----------------------------
async function fetchTokenPriceUSD(mint) {
  ...
  try {
    ...
    // getOnchainPrice expects a Number; clamp to safe integer if necessary
    const numericAmount = amountBig > BigInt(Number.MAX_SAFE_INTEGER) ? Number.MAX_SAFE_INTEGER : Number(amountBig);

    const quote = await getOnchainPrice(mint, USDC_MINT, numericAmount);
    const priceUSD = quote?.price ?? null;

    if (priceUSD && Number.isFinite(priceUSD)) {
      volCache[mint] = { lastPrice: priceUSD, ts: nowMs() };
      return priceUSD;
    } else {
    ...
    console.log("fetchTokenPriceUSD (on-chain) error:", err?.message || err);
    volCache[mint] = { lastPrice: null, ts: nowMs() };
    return null;
  }
}
