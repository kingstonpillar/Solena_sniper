...
// ---------------- MAIN EXPORT ----------------
export async function scanMintFast(tokenMint, solUsd = null) {
  ...
  // ---- Prepare pools for scanning ----
  const poolsByProgram = {
    raydium: POOL_KEYS, // all static Raydium pools
    orca: [],           // can populate dynamically on-chain later
    meteora: []
  };
  ...
  for (const pool of allPools) {
    if (pool.mintA !== mint && pool.mintB !== mint) continue;

    const baseIsA = pool.mintA === mint;
    const baseAmt = baseIsA ? pool.amountA : pool.amountB;
    const quoteAmt = baseIsA ? pool.amountB : pool.amountA;
    const quoteMint = baseIsA ? pool.mintB : pool.mintA;
