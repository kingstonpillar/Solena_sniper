...
export async function scanMintFast(mintAddress, boughtAt = null) {
  ...
  if (priceUSD) {
    const solUSD = await rpcQueue.add(() => fetchOraclePrice("So11111111111111111111111111111111111111112"));
    priceSOL = solUSD ? priceUSD / solUSD : null;

    // Overwrite price in JSON pool if exists
    const pools = readPools();
    const pool = pools.find(p => p.mintA === mintAddress || p.mintB === mintAddress);
    if (pool) {
      pool.price = priceSOL;
      writePools(pools);
    }
  ...
  if ((!priceSOL || !priceUSD) && boughtAt && now - boughtAt <= 6 * 60_000) {
    ...
    if (dexPrice) {
      priceUSD = dexPrice;
      const solUSD = await fetchDexScreenerPrice("So11111111111111111111111111111111111111112");
