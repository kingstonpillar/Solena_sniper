// dexBuilders.js
// AMM-only builder (Raydium v4 + Orca AMM) with cached pool scan
// - Removes Meteora
...
import { decodeWhirlpool } from "./orcaWhirlpoolUtils.js"; // still used, but AMM-only

const DEFAULT_SCAN_TTL_MS = Number(process.env.POOL_SCAN_TTL_MS || 60_000);
let poolCache = { ts: 0, payload: {} };

// ----------------------------------------------------------
// Cached scanPools for AMM-only programs
// ----------------------------------------------------------
async function scanPoolsCached(programPubkey) {
  ...
  if (
    poolCache.ts &&
    now - poolCache.ts < DEFAULT_SCAN_TTL_MS &&
    poolCache.payload &&
    poolCache.payload[programPubkey.toString()]
  ) {
    return poolCache.payload[programPubkey.toString()];
  }

  try {
    const poolAccounts = knownPoolAccounts[programPubkey.toString()] || [];
    const pools = await scanPools(poolAccounts); // AMM-only scan
    poolCache.ts = now;
    poolCache.payload = poolCache.payload || {};
    poolCache.payload[programPubkey.toString()] = pools || [];
    return poolCache.payload[programPubkey.toString()];
  } catch (err) {
    console.warn(`scanPoolsCached error for ${programPubkey.toBase58()}:`, err?.message || err);
    return poolCache.payload?.[programPubkey.toString()] || [];
  }
}
...
export async function findPoolsForPair(connection, mintIn, mintOut) {
  ...
  const dexList = [
    ...
    { dex: "orca", program: UPR_PROGRAMS.ORCA_WHIRLPOOL } // now treated as AMM-only
  ];
...
export async function selectDexForMint(mint) {
  ...
  const dexList = [
    { dex: "raydium", program: UPR_PROGRAMS.RAYDIUM_AMM },
    { dex: "orca", program: UPR_PROGRAMS.ORCA_WHIRLPOOL } // AMM-only
  ];
...
// ----------------------------------------------------------
// Compute dynamic on-chain slippage (Raydium & Orca AMM)
// ----------------------------------------------------------
export async function computeMinOut(connection, pool, amountIn, mintIn, slippageBps = 50) {
  ...
  if (pool.dex === "raydium" || pool.dex === "orca") {
...
// ----------------------------------------------------------
// Swap builders (Raydium & Orca AMM)
// ----------------------------------------------------------
...
export async function buildOrcaSwapTx(connection, walletPubkey, mintIn, mintOut, amountIn, options = { slippageBps: 50 }) {
  // AMM-only logic, no Whirlpool decoding
  const best = await findPoolsForPair(connection, mintIn, mintOut);
