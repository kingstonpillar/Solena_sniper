...
function createLimiter(maxPerSec) {
  ...
  function processQueue() {
    while (queue.length > 0 && active < maxPerSec) {
...
async function safeFetch(url, opts) {
  return limitHTTP(() => fetch(url, opts));
}
...
async function getCreatorsFromMetadata(mintPub) {
  try {
    ...
    need(1);
    const nameLen = data[idx]; need(1 + nameLen);
    idx += 1 + nameLen;

    need(1);
    const symbolLen = data[idx]; need(1 + symbolLen);
    idx += 1 + symbolLen;

    need(1);
    const uriLen = data[idx];
    need(uriLen);
    const uri = data.slice(idx + 1, idx + 1 + uriLen).toString("utf8").replace(/\0+$/g, "");
    idx += 1 + uriLen;
...
async function fetchMetadataUriCreators(uri) {
  try {
    ...
    const res = await safeFetch(uri, { timeout: 8000 }).catch(() => null);
