// liquidityGuard.js — Dynamic Liquidity + Full Protection
import fs from "fs";
...
const CHECK_INTERVAL = Number(process.env.CHECK_INTERVAL_MS || 15_000);
...
const positionLiquidity = new Map(); // Dynamic liquidity tracking
...
async function detectBigSell(mintAddress) {
  try {
    ...
    const sell = trades.reduce((s, t) => (t.side === "sell" ? s + (t.amount_usd || 0) : s), 0);
...
async function updateLiquidityReference(mintAddress, currentLiquidity) {
  const record = positionLiquidity.get(mintAddress) || { boughtLiquidity: currentLiquidity, highestLiquidity: currentLiquidity };
...
async function checkTokenPosition(pos) {
  ...
  try {
    ...
    const { highestLiquidity } = await updateLiquidityReference(mintAddress, currentLiquidity);
...
export async function monitorLiquidity() {
  try {
    ...
    for (const pos of positions) {
      try { await checkTokenPosition(pos); } catch {}
    }

    if (allSellsComplete()) console.log("🟢 All monitored tokens stable.");
  } catch (err) {
...
// ------------------- Start Interval -------------------
setInterval(() => {
  monitorLiquidity().catch(err => console.error("❌ monitorLiquidity uncaught error:", err.message));
}, CHECK_INTERVAL);
