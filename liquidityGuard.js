...
async function fetchOnchainPrice(mintAddress) {
  try {
    ...
    const result = await scanMintFast(conn, mintAddress, { retries: 1 });

    if (!result || !result.priceInSOL) {
      return { price: null, decimals };
    }

    return {
      price: result.priceInSOL,   // already the correct price
      decimals                   // keep just for UI display
    };

  } catch (err) {
    console.error(`scanMintFast price error for ${mintAddress}:`, err.message);
...
async function creatorStillSafe(mintAddress) {
  ...
  if (
    lastCreatorScan.has(mintAddress) &&
  ...
  try {
    return await verifyCreatorSafety(mintAddress); // ← AWAIT inside logic
  } catch {
...
async function checkTokenPosition(pos) {
  ...
  try {
    ...
    // FIXED: extract actual price number
    if (p && p.price != null) {
