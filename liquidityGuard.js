...
// =====================================================
// 1) UPDATED Telegram Alert (fix #2)
// =====================================================
...
async function detectBigSell(mintAddress) {
  try {
    ...
    const trades = (await res.json())?.data?.items || [];
    if (!trades.length) return false;

    const total = trades.reduce((s, t) => s + (t.amount_usd || 0), 0);
    if (!total) return false; // prevent divide-by-zero

    const sell = trades.reduce((s, t) => (
...
async function creatorStillSafe(mintAddress) {
  ...
  return await verifyCreatorSecurity(mintAddress);
}
...
export async function monitorLiquidity() {
  ...
  const positions = JSON.parse(
    fs.readFileSync(ACTIVE_POSITIONS_FILE, "utf8")
  );
