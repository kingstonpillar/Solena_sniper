...
// =======================
//   INIT JUPITER ONCHAIN
// =======================
// (AS YOU REQUESTED — initJupiter is awaited INSIDE logic)
let jupiterReady = false;

(async () => {
  try {
    await initJupiter({ connection: conn });   // ← AWAIT HERE
    jupiterReady = true;
    console.log("✔ Jupiter On-Chain Initialized");
  } catch (err) {
    console.error("❌ initJupiter failed:", err.message);
  }
})();
...
async function checkTokenPosition(pos) {
  ...
  try {
    ...
    await executeAutoSell(mintAddress, amount);  // ← AWAIT inside logic
