...
async function retry(fn, attempts = 6, backoff = 4000) {
  let last;
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      last = err;
      if (i < attempts - 1) {
        const delayMs = backoff * (i + 1); // linear backoff: 4000, 8000, 12000, ...
        await delay(delayMs);
      }
    }
  }
...
async function samplePriceForVolatility(mint, samples = 2, delayMs = 500) {
  try {
    ...
    for (let i = 0; i < samples; i++) {
      const p = await limit(() => scanMintFast(mint));
...
export async function executeSwap(inputMint, outputMint) {
  // --- Validate current trade amount ---
  if (!currentTradeAmount || currentTradeAmount <= 0) {
    console.log("currentTradeAmount not ready.");
    return null;
  }

  // --- Extra lamports for block delivery / slippage ---
  let extraLamports = 0;
  try {
    extraLamports = await computeUnitPerTrade();
  } catch (err) {
    console.error("computeUnitPerTrade failed:", err?.message || err);
  }

  // --- Total amount in lamports ---
  const amountRaw = Math.floor((currentTradeAmount + (extraLamports || 0)) * 1e9);
  console.log(
    `🪙 Swap request: ${currentTradeAmount} SOL + ${extraLamports || 0} extra → raw ${amountRaw}`
  );
  ...
  try {
    built = await limit(() =>
      buildSwapTx(conn, wallet.publicKey, inputMint, outputMint, amountRaw, {
    ...
    // --- Fallback: discover pools ---
    let found;
    try {
      found = await retry(() => limit(() => findPoolsForPair(conn, inputMint, outputMint)), 6, 4000);
    } catch (disErr) {
      console.error("Pool discovery error:", disErr?.message || disErr);
      return null;
    }

    if (!found?.pools?.length) {
  ...
  try {
    ...
    try {
      await limit(() => conn.confirmTransaction(signature, "confirmed"));
    } catch (e) {
      console.warn("ConfirmTransaction warning:", e?.message || e);
    }
  ...
  // --- Price fetch for active position logging ---
  let entryBuyPriceSOL = null;
  let entryBuyPriceUSD = null;

  try {
    const priceInfo = await limit(() => scanMintFast(outputMint));
    entryBuyPriceSOL = priceInfo?.priceInSOL ?? priceInfo?.price ?? null;
    entryBuyPriceUSD = priceInfo?.priceInUSD ?? null;
  } catch (err) {
    console.log(`scanMintFast error for ${outputMint}:`, err?.message || err);
  }
  ...
  try {
    const entry = {
      ...
      buyPriceUSD: entryBuyPriceUSD,
      amount: currentTradeAmount + (extraLamports || 0),
    ...
    console.log(
      `Saved active buy for ${outputMint}: SOL=${entryBuyPriceSOL}, USD=${entryBuyPriceUSD}`
    );

    await sendTelegram(
      `🚀 *BUY EXECUTED*\nMint: ${outputMint}\nAmount: ${currentTradeAmount + (extraLamports || 0)} SOL\nSig: https://solscan.io/tx/${signature}`
    );
  } catch (err) {
