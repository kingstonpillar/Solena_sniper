...
export async function executeSwap(inputMint, outputMint) {
  ...
  try {
    ...
    try {
  const found = await retry(() => limit(() => findPoolsForPair(conn, inputMint, outputMint)), 3, 1200);
  if (!found || !found.pools || found.pools.length === 0) {
    console.error("No candidate pools found on-chain for pair.");
    return null;
  }
  // pool is now safely in correct direction
  const pool = found.pools[0];
  // ... proceed to buildSwapTx with poolHint = pool if needed
} catch (err) {
  console.error("Pool discovery error:", err?.message || err);
  return null;
}
      let success = false;
      for (const p of found.pools) {
        ...
        try {
          built = await retry(() =>
  limit(() => buildSwapTx(conn, wallet.publicKey, inputMint, outputMint, amountRaw, {
    slippageBps: dynamicSlippageBps,
    poolHint: p,
    mode: MODE
  })), 3, 1200
...
try {
  ...
  entryBuyPriceSOL = null;
  entryBuyPriceUSD = null;
}
