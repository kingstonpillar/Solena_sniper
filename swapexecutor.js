...
function getWalletFromEnvEncrypted() {
  ...
  // 🔐 DEBUG: Confirm wallet successfully decrypted
...
export async function executeSwap(inputMint, outputMint) {
  ...
  // ------------------------------------------------
  // ------------------------------------------------
// ⭐ DRY-RUN CHECK — DO NOT EXECUTE REAL SWAP
// ------------------------------------------------
if (process.env.DRY_RUN === "true") {
  ...
  console.log("🧪 Route summary:", {
    ...
    expectedOutput: route.outAmount / (10 ** (route.outDecimals || 0)),
  });

  return {
    dryRun: true,
    routeUsed: route,
    message: "Dry-run completed. No on-chain transaction executed."
  };
}

// ------------------------------------------------
// ⭐ REAL EXECUTION — Jupiter on-chain swap
// ------------------------------------------------
let execResult;
try {
...
try {
  const outputSymbol =
    route?.outToken?.symbol ??
    route?.outTokenAddress ??
    "UNK";
