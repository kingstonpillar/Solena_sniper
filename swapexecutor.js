...
// -------------------- SWAP EXECUTION --------------------
export async function executeUnsignedSwaps() {
  ...
  for (const txData of unsignedTxs) {
    ...
    try {
      // Adjust transaction with trade amount
      const adjustedInstructions = txData.instructions.map(ix => {
        // Here we assume the instruction has a 'lamports' or 'amount' field
        // Replace it with currentTradeAmount (SOL)
        if (ix?.lamports !== undefined) ix.lamports = Math.round(currentTradeAmount * 1e9);
        return ix;
      });

      const tx = new Transaction().add(...adjustedInstructions);
