...
// Rate limit to 6 requests per second
...
export async function executeUnsignedSwaps() {
  ...
  for (const txData of unsignedTxs) {
    ...
    try {
      const tx = new Transaction().add(...txData.instructions);

      // Send via RPC retry + rate-limit
      ...
      // If all sells complete, restart watcher
      const ready = await allSellsComplete();
      if (ready && !watcherActive) {
        await startWatcher();
      }
