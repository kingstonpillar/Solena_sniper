...
export let computeUnitPerTrade = computeUnitPerTx;
...
export async function computeTradeAmount() {
  ...
  if (!sold) return; // ensure not to break buy/sell cycle
...
// =============== BALANCE HEARTBEAT (every 30 min) ===============
async function sendBalanceHeartbeat() {
  const bal = await getWalletBalance();

  console.log(`[Heartbeat] Balance: ${bal.toFixed(6)} SOL | Trade Amount: ${currentTradeAmount}`);
...
async function startLoop() {
  ...
  sendBalanceHeartbeat(); // run once immediately

  setInterval(sendBalanceHeartbeat, 30 * 60 * 1000); // repeat
}
