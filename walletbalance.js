...
export let computeUnitPerTrade = 0; // initialize to 0
...
async function sendBalanceHeartbeat() {
  ...
  fs.writeFileSync(
    TRADE_FILE,
    JSON.stringify(
      { tradeAmount: currentTradeAmount, balance: bal, timestamp: Date.now() },
      null,
      2
    )
  );
...
async function startLoop() {
  ...
  setInterval(sendBalanceHeartbeat, 30 * 60 * 1000); // repeat every 30 min
}
